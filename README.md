...
## 1.覚えることと、脳内で暗算すること。
1. メジャースケールで考える。マイナースケールは覚えずに、「平行短調はキーが短三度下」から暗算する。「短三度」は「ひとつ飛ばしの白鍵同士の間に黒鍵が１個しかない場合」であることや、「指板の６弦５フレットAと６弦８フレットCの距離」であることを視覚的イメージで覚える。
2. 「#」が１個だけ付くのは、「移動ド」を「G」にしたとき。「移動シ」にあたる「F」に「#」が付く。鍵盤で移動ドをGで弾いてみる。これはしっかり暗記する。
3. 「♭」が１個だけ付くのは、「移動ド」を「F」にしたとき。「移動ファ」にあたる「B」に「♭」が付く。鍵盤で移動ドをFで弾いてみる。これはしっかり暗記する。
4. 「#」が増えるにしたがって、キーは「G、D、A、E、B」、「#」が付く位置は「F、C、G、D、A」と現れる。鍵盤でこの出現順を視覚的に覚える。Cも含めると、キーは白鍵ばかり６つ。
5. 「♭」が増えるにしたがって、キーは「F、B♭、E♭、A♭、D♭、G♭」、「♭」が付く位置は「B、E、A、D、G、C」と現れる。鍵盤で視覚的にこの出現順を覚える。キーはFと黒鍵全部の合計６つ。
6. 鍵盤で実際に、１２通りのキーで移動ドのドレミファソラシドを弾いて、上記を確認する。
  
- 一覧イメージ図  
![image](./KeySignature.png)
  
## 2.とにかく「黒鍵」を１回だけ使うキーを把握する
...
まずはこのような、「いずれの間にも黒鍵がある、隣接した３音」、すなわち「長３度」がないかと見渡すと、ほかに２か所あります。  
「FGA」と「GAB」です。  
それぞれ、くわしく観察してみましょう。  
...
ピアノの鍵盤で、「白鍵同士で「長３度」なところ」の３か所（C->E）（F->A）（G->B）を視覚的に暗記することが、スタート地点です。
...
イメージできるでしょうか。  
さらに、「#」が付く位置も、「黒鍵２個側」と「黒鍵３個側」に交互に、昇順に現れて、積み上げていく感じです。  
- 「#」が付く位置
  - ①側：_C_D  
  - ②側：F_G_A  
...
「♭」が付く位置も、「黒鍵２個側」と「黒鍵３個側」に交互に、降順に現れて、積み上げていく感じです。  
- 「♭」が付く位置
  - ①側：_E_D_C  
  - ②側：B_A_G  
